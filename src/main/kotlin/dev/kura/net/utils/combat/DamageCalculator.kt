...
object DamageCalculator: Util() {
    ...
    // Anchor damage
    fun anchorDamage(player: LivingEntity, targetPos: Vec3d, box: Box, anchor: BlockPos): Double {
...
//        return anchorDamage(target, targetPos, targetBox, crystal)
            ...
            if (distExposure <= 1.0) {
                ...
                if (diff != 0.0) {
                    ...
                    if (toDamage <= 0) {
                        ...
                        if (protectAmount > 0) {
                            toDamage = DamageUtil.getInflictedDamage(toDamage.toFloat(), protectAmount.toFloat()).toDouble()
                        }
                    }
                    return toDamage
                }
    ...
    // Utils
    private fun getDamageForDifficulty(damage: Double): Double {
    ...
    private fun blastProtReduction(player: Entity, damage0: Double, explosion: Explosion): Double {
        var damage = damage0
        var protLevel =
...
//                        (vec3d as IVec3d) [n + g, o] = p + h
    ...
    fun raycast(context: RaycastContext): BlockHitResult {
