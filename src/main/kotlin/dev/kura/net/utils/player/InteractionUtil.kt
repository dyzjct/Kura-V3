...
object InteractionUtil : Util() {

    var awaiting: MutableMap<BlockPos, Long> = HashMap()

    fun findBestItem(pos: BlockPos, inventory: Boolean): Item? {
        var slot = -1
        var item: Item? = null
    ...
    fun findBestSlot(pos: BlockPos, inventory: Boolean): Int? {
        var slot = -1
        for (i in 0 until if (inventory) 46 else 9) {
            if (slot == -1 || player.inventory.getStack(i)
                    .getMiningSpeedMultiplier(world.getBlockState(pos)) > player.inventory.getStack(
    ...
    fun placeBlock(
        ...
        PlaceRender.renderBlocks[bp] = System.currentTimeMillis()
        val result = getPlaceResult(bp, interact, ignoreEntities) ?: return false
        val sprint = player.isSprinting
        val sneak = needSneak(mc.world!!.getBlockState(result.blockPos).block) && !player.isSneaking
        ...
        if (mode == PlaceMode.Normal) playerController.interactBlock(mc.player, Hand.MAIN_HAND, result)
    ...
    fun canPlaceBlock(bp: BlockPos, interact: Interact, ignoreEntities: Boolean): Boolean {
        return if (awaiting.containsKey(bp)) false else getPlaceResult(bp, interact, ignoreEntities) != null
    }

    fun getPlaceResult(bp: BlockPos, interact: Interact, ignoreEntities: Boolean): BlockHitResult? {
        if (!ignoreEntities) {
        ...
        for (support in supports) {
            if (interact != Interact.Vanilla) {
                if (RotationUtil.getStrictDirections(bp).isEmpty()) return null
                if (!RotationUtil.getStrictDirections(bp).contains(support.facing)) continue
            }
    ...
    fun getSupportBlocks(bp: BlockPos): ArrayList<BlockPosWithFacing> {
    ...
    @Suppress("DEPRECATION")
    fun checkNearBlocks(blockPos: BlockPos): BlockPosWithFacing? {
    ...
    private val shiftBlocks: List<Block> = listOf(
        Blocks.ENDER_CHEST,
        Blocks.CHEST,
    ...
    fun getVisibleDirectionPoint(dir: Direction, bp: BlockPos): Vec3d? {
            ...
            while (y < brutBox.maxY) {
                ...
                while (z < brutBox.maxZ) {
                    ...
                    val wallCheck = world.raycast(
                        RaycastContext(
                            RotationUtil.getEyesPos(mc.player),
                            point,
