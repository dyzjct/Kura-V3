...
object CrystalUtil : dev.m7thh4ck.net.util.Util() {
        ...
        for (x in startX..endX) {
            for (z in startZ..endZ) {
                for (y in startY until endY) {
                    val dist =
                        ((cx - x).sq + (cz - z).sq + if (sphere) (cy - y).sq else 0).toDouble()
                    if (dist < rSquared && !(hollow && dist < rMinusOneSquared)) {
    ...
    fun canPlaceCrystal(pos: BlockPos): Boolean {
        for (target in world.entities) {
            ...
            if (getCrystalPlacingBB(pos)
                    .intersects(target.boundingBox)
            ) return false
        }
        return canPlaceCrystalOn(pos) && hasValidSpaceForCrystal(
            pos
        )
    }
    ...
    fun canPlaceCrystal(pos: BlockPos, entity: LivingEntity? = null): Boolean {
        return canPlaceCrystalOn(pos)
                && (entity == null || !getCrystalPlacingBB(pos)
    ...
    fun hasValidSpaceForCrystal(pos: BlockPos): Boolean {
        val mutableBlockPos = dev.m7thh4ck.net.util.combat.CrystalUtil.mutableBlockPos.get()
        return isValidMaterial(
            world.getBlockState(
                mutableBlockPos.set(pos).add(0, 1, 0)
            )
        )
                && isValidMaterial(
            world.getBlockState(
    ...
    fun crystalPlaceBoxIntersectsCrystalBox(placePos: BlockPos, crystalPos: Vec3d): Boolean {
        return crystalPlaceBoxIntersectsCrystalBox(
            placePos,
            crystalPos.x,
            crystalPos.y,
            crystalPos.z
    ...
    fun crystalPlaceBoxIntersectsCrystalBox(
        ...
        return (crystalY.fastFloor() - placePos.y).withIn(0, 2)
                && (crystalX.fastFloor() - placePos.y).withIn(-1, 1)
    ...
    fun placeBoxIntersectsCrystalBox(placePos: BlockPos, crystal: EndCrystalEntity): Boolean {
        return placeBoxIntersectsCrystalBox(
            placePos,
            crystal.x,
            crystal.y,
            crystal.z
    ...
    fun isResistant(blockState: BlockState) =
        !blockState.isLiquid && blockState.block.blastResistance >= 600
