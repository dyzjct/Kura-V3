...
object VectorUtils {
    ...
    fun getBlockPosInSphere(center: Vec3d, radius: Float): ArrayList<BlockPos> {
        ...
        for (x in getAxisRange(center.x, radius)) for (y in getAxisRange(center.y, radius)) for (z in getAxisRange(
            ...
            if (blockPos.distanceSqTo(center.x, center.y, center.z) > squaredRadius) continue
    ...
    fun Vec3i.toBlockPosCenter(): BlockPos {
        return BlockPos(toVec3dCenter(0.0, 0.0, 0.0).toBlockPos())
    }
