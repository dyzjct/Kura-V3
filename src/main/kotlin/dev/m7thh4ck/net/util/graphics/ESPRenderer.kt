...
import dev.m7thh4ck.net.util.graphics.mask.DirectionMask
...
class ESPRenderer {
    ...
    fun add(
        pos: BlockPos,
        color: ColorRGB,
        outlineColor: Color = Color(color.r, color.g, color.b, color.a),
        sides: Int
    ) {
        add(Box(pos), color, outlineColor, sides)
    }
    ...
    fun render(stack: MatrixStack, clear: Boolean) {
        ...
        if (filled) {
            for ((box, color, _) in toRender0) {
                ...
                Render3DEngine.drawFilledBox(box, Color(c.red, c.green, c.blue, a))
            }
        }
        ...
        if (outline) {
            for ((box, color, _) in toRender0) {
                ...
                Render3DEngine.drawBoxOutline(box, Color(c.red, c.green, c.blue, a), 1f)
            }
        }
